...
- (void)updateNavigation {
    ...
    _actionButton.enabled = YES;//[[self photoAtIndex:_currentPageIndex] underlyingImage] != nil;
	self.navigationItem.rightBarButtonItem = _actionButton;
}
...
#pragma mark - Actions  改写-------删除图片------

- (void)actionButtonPressed:(id)sender {
    
    UIActionSheet *sheet = [[UIActionSheet alloc]initWithTitle:@"要删除这张照片吗" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"删除" otherButtonTitles: nil];//[[UIActionSheet alloc]initWithTitle:@"要删除这张照片吗" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"删除" otherButtonTitle:@"",nil];
    sheet.tag = ACTION_SHEET_OLD_ACTIONS;
    ...
    if (actionSheet.tag == ACTION_SHEET_OLD_ACTIONS) {
        ...
        if(buttonIndex == 0){
            if([self.delegate respondsToSelector:@selector(photoBrowser:didDeletePhotoAtIndex:)]){
                ...
                [self.delegate photoBrowser:self didDeletePhotoAtIndex:_currentPageIndex];
            }
        }
    }
